...
namespace gem5
{
  ...
  namespace prefetch
  {
     ...
     */
    class TriangelHashedSetAssociative : public TaggedSetAssociative
    {
    public:
      ...
      TriangelHashedSetAssociative(
          ...
          : TaggedSetAssociative(p), ways(0), max_ways(8)
      {
      }
    ...
    class Triangel : public Queued
    {
      ...
      struct TrainingUnitEntry : public TaggedEntry
      {
        ...
        TrainingUnitEntry(TagExtractor ext)
          : TaggedEntry(), lastLastAddress(0), local_timestamp(0), reuseConfidence(4, 8), patternConfidence(4, 8), highPatternConfidence(4, 8), replaceRate(4, 8), hawkConfidence(4, 8), lastAddressSecure(false), lastLastAddressSecure(false), currently_twodist_pf(false)
        {
    ...
    public:
      struct SizeDuel
      {
        ...
        void reset(uint64_t mask, uint64_t modMax, uint64_t cacheAssoc)
        {
          ...
          set = rng->random<uint64_t>(0, setMask);
          temporalMod = rng->random<uint64_t>(0, modMax - 1); // N-1, as range is inclusive.
        }
      ...
      struct Hawkeye
      {
        ...
        void reset()
        {
          ...
          set = rng->random<uint64_t>(0, setMask - 1);
        }

        void decrementOnLRU(Addr addr, AssociativeCache<TrainingUnitEntry> *trainer)
        {
          if ((addr % setMask) != set)
            return;
        ...
        void add(Addr addr, Addr pc, AssociativeCache<TrainingUnitEntry> *trainer)
        {
          if ((addr % setMask) != set)
            return;
      ...
      struct MarkovMapping : public TaggedEntry
      {
        ...
        MarkovMapping(TagExtractor ext) :
          TaggedEntry(), index(0), address(0), confident(false), cycle_issued(0)
        {
          registerTagExtractor(ext);
        }
      ...
      struct SampleEntry : public TaggedEntry
      {
        ...
        SampleEntry(TagExtractor ext) :
          TaggedEntry(), entry(nullptr), reused(false), local_timestamp(0), next(0)
        {
          registerTagExtractor(ext);
        }
      ...
      AssociativeCache<SampleEntry> historySampler;

      /** Test pf entry, tagged by data address**/
      struct SecondChanceEntry : public TaggedEntry
      {
        SecondChanceEntry(TagExtractor ext) : TaggedEntry(), global_timestamp(0), used(false)
        {
          registerTagExtractor(ext);
        }
        Addr pc;
      ...
      AssociativeCache<SecondChanceEntry> secondChanceUnit;

      /** History mappings table */
      AssociativeCache<MarkovMapping> markovTable;
      static AssociativeCache<MarkovMapping> *markovTablePtr;

      AssociativeCache<MarkovMapping> metadataReuseBuffer;
